...
- **Corrections** — if the public record has moved (a postmortem published, an attribution made, a retraction issued), open an issue or PR with the source. Corrections are the most valuable contribution.
...
3. **State the investigation status honestly.** "No formal investigation was conducted or published" is a finding.
4. **Date everything.** Incident date, source dates, and the file's own *last reviewed* date.

## Process

Open an issue first for new case files or structural changes; PRs directly for corrections and link additions. Be kind in review — many contributors here come from outside software and are using GitHub for the first time. The project deliberately welcomes them.
